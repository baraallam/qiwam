  ...
  return (
    <div dir={t.dir} style={{ minHeight: "100vh", background: C.bg, fontFamily: FONT, color: C.ink }}>
      ...
      <header style={{ background: C.pine, color: "#fff" }}>
        <div style={{ maxWidth: 1060, margin: "0 auto", padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
      ...
      <nav style={{ background: "#fff", borderBottom: `1px solid ${C.line}` }}>
        <div style={{ maxWidth: 1060, margin: "0 auto", padding: "0 20px", display: "flex", gap: 4 }}>
          {[["calculator", t.pageCalc], ["spending", t.pageSpend], ["learn", t.pageLearn]].map(([k, label]) => (
            <button key={k} onClick={() => go(k)}
              style={{
                fontFamily: FONT, fontSize: 14.5, fontWeight: page === k ? 700 : 500, cursor: "pointer",
      ...
      {page === "calculator" && (
        <div style={{ maxWidth: 1060, margin: "18px auto 0", padding: "0 20px", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          {subTabs.map(([k, label]) => (
      ...
      <main style={{ maxWidth: 1060, margin: "18px auto 60px", padding: "0 20px" }}>
        {/* ---------------- PROFILE ---------------- */}
            ...
            <section style={{ ...card, background: C.pine, border: "none", color: "#fff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
                <div>
                  ...
                  <div style={{ fontSize: 38, fontWeight: 700, lineHeight: 1.15, fontVariantNumeric: "tabular-nums", color: remainingM < 0 ? "#F0A08C" : "#fff" }}>
                    {fmt(remainingM, lang)} <span style={{ fontSize: 16, fontWeight: 500 }}>{t.sar}</span>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 24px", alignContent: "center" }}>
                  {[[t.heroIncome, fmt(monthIncome, lang) + " " + t.sar], [t.heroSpent, fmt(daily.total, lang) + " " + t.sar],
                    ...
                    <div key={l}>
                      ...
                      <div style={{ fontSize: 16.5, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{v}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {page === "calculator" && tab === "results" && (
          <div style={{ display: "grid", gap: 16 }}>
            {/* score + KPIs */}
            <div style={{ display: "grid", gridTemplateColumns: "minmax(240px, 300px) 1fr", gap: 16, alignItems: "stretch" }}>
        ...
        {page === "spending" && (
          <div style={{ display: "grid", gap: 16, marginTop: 16 }}>
            <section style={card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, margin: "14px 0 18px" }}>
        ...
        {page === "learn" && (
          <div style={{ display: "grid", gap: 16 }}>
            <section style={{ ...card, background: C.goldSoft, borderColor: "#E4D6B8" }}>
              <h1 style={{ fontSize: 21, fontWeight: 700, color: C.pine, margin: "0 0 8px" }}>{t.learnH}</h1>
