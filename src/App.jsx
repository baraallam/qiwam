...
function Auth({ t, lang, setLang, onLogin }) {
  ...
  async function submit() {
    ...
    if (mode === "register") {
      ...
      if (data.user) {
        onLogin({ email: data.user.email, name: data.user.user_metadata?.full_name || data.user.email });
      }
    }
  }
...
/* ------------------------------------------------------------------ */
/*  Manual Content Component (Shortened for brevity, keep same as before) */
/* ------------------------------------------------------------------ */
function ManualContent() {
  // (Keep your existing ManualContent code here exactly as it was)
  return <div>دليل المستخدم</div>; // Placeholder to not break code
}
...
export default function App() {
  ...
  // Fixed loginDone to ensure user id is fetched correctly
